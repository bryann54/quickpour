...
class PromotionsCarousel extends StatelessWidget {
    ...
    return BlocBuilder<PromotionsBloc, PromotionsState>(
      builder: (context, state) {
        if (state is PromotionsLoading) {
          ...
          return _buildCarousel(state.products);
        } else {
  ...
  Widget _buildCarousel(List<ProductModel> products) {
    ...
    products.sort((a, b) {
      ...
      return discountB.compareTo(discountA); // Descending order
    });

    // Optionally, limit the number of products shown in the carousel
    final topDiscountedProducts = products.take(5).toList(); // Show top 5
